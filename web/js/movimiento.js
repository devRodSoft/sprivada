...
$('body').on("keypress", '.cantidad , .costo', function(e) {
...
$('body').on("blur", '.cantidad , .costo', function(e) {
   totalItem($(this));
});
    ...
    $.get("/erp/almacen/verifycode", {  code: valor })
      .done(function( data ) {
        if(data.error == 1){
          ...
          siguiente.next().find('input').focus();
        }
    });//TERMINA FUNCION DONE
}


$('input[name="Movimiento[stpagado]"]').on('switchChange.bootstrapSwitch', function(event, state) {
    if(state){
        $(".field-movimiento-metodo").show('slow');
    }else{
        $(".field-movimiento-metodo").hide('slow');
    }
});
...
$('input[name="Movimiento[stproyecto]"]').on('switchChange.bootstrapSwitch', function(event, state) {
    if(state){
        $(".proy").show('slow');
    }else{
           $(".proy").hide('slow');
